...
$$
maxDp[i] = max\{nums[i], \, maxDp[i - 1] \times nums[i], \, minDp[i - 1] \times nums[i]\} \\ minDp[i] = min\{nums[i], \, maxDp[i - 1] \times nums[i], \, minDp[i - 1] \times nums[i]\}
$$
...
$$
dp[i] = max\{dp[i], \, dp[j] + 1\} \\ \text {if \, j < i \, \& \, nums[i] > nums[j] }
$$
...
$$
up[i] = \begin{cases} down[i - 1] + 1, & \text {nums[i - 1] < nums[i]} \\ up[i], & \text{nums[i - 1] $\geq$ nums[i]} \end{cases}
$$

$$
down[i] = \begin{cases} down[i - 1], & \text {nums[i - 1] $\leq$ nums[i]} \\ up[i - 1] + , & \text{nums[i - 1] > nums[i]} \end{cases}
$$
