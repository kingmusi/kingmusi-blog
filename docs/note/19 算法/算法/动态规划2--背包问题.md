...
$$
dp[i][j] = \begin{cases} dp[i - 1][j], & \text {j < v[i]} \\ max\{dp[i - 1][j], \, dp[i - 1][j - v[i]] \}, & \text{j $\geq$ v[i]} \end{cases}
$$
...
$$
dp[i][j] = \begin{cases} dp[i - 1][j], & \text {j < nums[i]} \\ dp[i - 1][j] \, || \, dp[i - 1][j - v[i]], & \text{j $\geq$ nums[i]} \end{cases}
$$
...
$$
dp[i][j][k] = \begin{cases} dp[i - 1][j][k], & \text {j < 0的个数 \&\& k < 1的个数} \\ max\{dp[i - 1][j][k], \,\, dp[i - 1][j - 0的个数][k - 1的个数] + 1 \}, & \text{j $\geq$ 0的个数 \&\& k $\geq$ 1的个数} \end{cases}
$$
